module ApplicationHelper
  ...
  def figure_filter(i,f)
    ...
    if i[0] == :associations
      ...
      html += figure_asso(i,f)

    elsif i[1] == 0 || i[1] == "datepicker"
  ...
  def figure_asso(i,f)
    ...
    i[1].each do |c|
      ...
      html += "<div class='#{c[:title]}' style='display: none;'>" 
      html += "<div class='clear'></div>"    
      html += "<h5 class='bold-subhead'> #{c[:title]}</h5>"
        ...
        c[:cols].each do |j|
          ...
          if j[1] == 0 || j[1] == "datepicker"
            ...
            html += h_datepicker(f,j[0],j[1],j[0])          
            ...
            html += figure_date_range(j,f)    

          end
        end
      html += '</div>'  
      l << link_to(c[:title],"#",{onclick: "$('.#{c[:title]}').slideToggle();"}).html_safe
    end
    [html.html_safe,l]
  end
  #===============================================================
  #== deals with creating text fields for date with from and to ==
  def figure_date_range(i,f)
        html = "" 
        html += "<div class='clear'></div>"    
        html += "<h5 class='bold-subhead'> #{i[1][:title]}</h5>"

        if i[1][:type] == "daterange"
          ...
          html += h_datepicker(f,d,"datepicker<",i[1][:ph])
          html += h_datepicker(f,d,"datepicker>",i[1][:ph])
  ...
  def h_datepicker(f,d,dir,ph)
    ...
    html += "<div class='row'>"
    html += f.text_field field,{class: "form-control datepicker", placeholder: ph.to_s.titleize + ph1} 
    html += "</div>"
    html
  end
